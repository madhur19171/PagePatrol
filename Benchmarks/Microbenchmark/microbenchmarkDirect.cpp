...
public:
    std::string name;
    virtual void execute(int fd, size_t num_pages) = 0;
    virtual ~AccessPattern() = default;
};

// Sequential access with gaps (we only access one byte per page)
...
public:
    ...
    void execute(int fd, size_t num_pages) override {
        ...
        off_t offset = 0; 
        while (offset < PAGE_SIZE * num_pages) {
            ...
            if (bytesRead == -1) {
                std::cerr << "Failed to read, error code:" << bytesRead << std::endl;
                free(buffer);
                return;
            } 

            offset += gap * PAGE_SIZE;
        }
...
public:
    ...
    void execute(int fd, size_t num_pages) override {
        ...
        int numPagesToRead = (num_pages * region_size) / 100;
        int startPage = rand() % (num_pages - numPagesToRead);

        for (int i = 0; i < repeats; i++) {

            int randomPage = startPage + (rand() % numPagesToRead);
            off_t offset = PAGE_SIZE * randomPage; 
            if (lseek(fd, offset, SEEK_SET) == -1) {
                std::cerr << "Failed to seek" << std::endl;
                free(buffer);
                return;
...
void run_access_patterns(int fd, size_t num_pages, const Config& config,
    ...
    while (true) {
        for (const auto& pattern_name : config.execution_sequence) {
            ...
            if (it != patterns_map.end()) {
                ...
                pattern->execute(fd, num_pages);  // Execute the pattern
            } else {
...
int main(int argc, char* argv[]) {
    ...
    int fd = open(config.file_name.c_str(), O_RDONLY | O_DIRECT);
    if (fd < 0) {
    ...
    run_access_patterns(fd, num_pages, config, patterns_map);
