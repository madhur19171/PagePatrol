...
#define MAX_FREQUENCY_ENTRIES 50
#define MAX_PINNED_PAGES 7
#define MAX_HEAP_HEIGHT 3
#define BLOCK_SIZE 4096
#define NUM_BLOCK_PER_NODE 2500
...
struct {
    ...
    __type(key, __u64); // Key is the index
...
int BPF_KPROBE(__task_pid_nr_ns, struct task_struct *task, enum pid_type type, struct pid_namespace *ns) {
    ...
    __u64* frequecy_map_size_ptr = bpf_map_lookup_elem(&va_frequency_array, &zero64);
    __u64 frequecy_map_size = frequecy_map_size_ptr ? *frequecy_map_size_ptr : 0;
    if (!frequecy_map_size_ptr) {
        bpf_map_update_elem(&va_frequency_array, &zero64, &frequecy_map_size, BPF_ANY);
    }
    ...
    for (i = 0; i < MAX_PID_VA_ENTRIES; i++) {
		idx = i;	// This is necessary to avoid infinite loop error. Otherwise, if &i is directly passed to bpf_map_lookup_elem, 
    ...
    for(i = 1; i < MAX_FREQUENCY_ENTRIES; i++) {
        ...
        if (heap_size < MAX_PINNED_PAGES) {
            ...
            //bpf_printk("end buugle up\n");
            // end of bubble up
            heap_size++;
        } else {
            min_va = bpf_map_lookup_elem(&min_heap, &zero64);
            if (!min_va) {
                return 0;
            }
            ...
            if (*min_freq < *current_freq) {
                bpf_map_update_elem(&min_heap, &zero64, va, BPF_ANY);

                // bubble down
                index = 0;
                #pragma unroll
                for (j = 0; j < MAX_HEAP_HEIGHT; j++) {
                    ...
                    if (left_child_frequency && *left_child_frequency < *current_frequency) {
                        smallest = left_child_index;
                        smallest_frequency = *left_child_frequency;
                    }

                    if (right_child_frequency && *right_child_frequency < smallest_frequency) {
    ...
    __u64* pinned_va, *heap_va;
    __u64 addr;
    __u64 keep, heap_index;
    ...
    for (i = 0; i < MAX_PINNED_PAGES; i++) {
        ...
        keep = 0;
        for (j = 0; j < MAX_PINNED_PAGES; j++) {
            if (j >= heap_size) {
                break;
            }
            heap_index = j;
            heap_va = bpf_map_lookup_elem(&min_heap, &heap_index);
            if (heap_va) {
                keep = 1;
                break;
            }
    ...
    for (i = 0; i < MAX_PINNED_PAGES; i++) {
        ...
        for (j = 0; j < NUM_BLOCK_PER_NODE; j++) {
            addr = (*pinned_va + (j * BLOCK_SIZE));
            bpf_unpin_file_vaddr(pid, addr);
        }
